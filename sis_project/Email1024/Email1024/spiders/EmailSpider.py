...
class EmailspiderSpider(scrapy.Spider):
    ...
    def parse_block_page(self, response):
        ...
        temp_list = soup.find_all('a', attrs={'href': True, 'style': "font-weight: bold;color: blue", 'id': False})
        for item in temp_list:
            if "html" not in item['href']:
                continue
            topic_id = item['href'].split('-')[1]
            if int(topic_id) < 10240000:
                continue
    ...
    def parse_poster_page(self, response):
        ...
        print(topic_title)
        img_list = soup.find_all('img', attrs={'border': 0, 'onclick': "zoom(this)", 'onload': "attachimg(this, 'load')"})
        topic_img_list = list()
        image_count = 0
        ...
        b_list = soup.find_all('a', attrs={'id': True, 'class': "dropmenu"})
        ...
        fileItem['topic_img_url'] = topic_img_list
        fileItem['author_name'] = author_name
        urlList = [torrent_download_url]
        for img_url in topic_img_list:
